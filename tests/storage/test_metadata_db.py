...
方式1 - 通过 pytest（使用默认配置）：
    pytest tests/storage/test_metadata_db.py -v

方式2 - 直接运行（使用 argparse 参数）：
    python tests/storage/test_metadata_db.py --config-path=src/config/config_tecent_backend_server_test.yaml
...
def get_config_path_from_args() -> Path:
    ...
    if not config_path.exists():
        raise ValueError(
            ...
            f"请通过以下方式之一指定配置文件：\n"
            f"  1. 命令行参数: python tests/storage/test_metadata_db.py --config-path=src/config/config_tecent_backend_server_test.yaml\n"
