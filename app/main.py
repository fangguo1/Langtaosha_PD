...
# 默认 tests 夹具配置；生产请设置环境变量 PD_TEST_CONFIG 或将 YAML 放入 local_data/
...
def api_stats():
    ...
    try:
        ...
        # 使用当前配置的数据库键名
        engine = get_db_engine(db_key=_db_key)
        ...
        with engine.connect() as conn:
            ...
            result = conn.execute(text("""
                ...
                ORDER BY source
            """))
            source_stats = {row[0]: row[1] for row in result.fetchall()}
            arxiv_count = source_stats.get('arxiv', 0)
            pubmed_count = source_stats.get('pubmed', 0)
