...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify the real expanded_sparse /api/search endpoint.",
    )
    ...
    parser.add_argument(
        "--source-list",
        default=os.environ.get("SOURCE_LIST"),
    ...
    parser.add_argument(
        ...
        default="0",
        choices=("0", "1"),
        help="Pass include_coverage to the API, default: %(default)s",
    )
    parser.add_argument(
        "--include-loose-coverage",
        default="0",
        choices=("0", "1"),
        help="Pass include_loose_coverage to the API, default: %(default)s",
    )
    parser.add_argument(
        "--timeout",
        type=float,
    ...
    parser.add_argument(
        "--request-id",
        default=None,
        help="Optional X-Request-Id header. Defaults to a generated UUID4 hex.",
    )
    parser.add_argument(
        "--auth-token",
...
def build_request_url(args: argparse.Namespace) -> str:
    params = {
        ...
        "include_loose_coverage": args.include_loose_coverage,
    }
    if args.source_list:
...
def main(
    ...
    *,
    ...
    stdout.write("== expanded_sparse request ==\n")
    stdout.write(f"url: {url}\n")
    stdout.write(f"request_id: {headers['X-Request-Id']}\n")
    stdout.write("note: server-side expanded_sparse_search logging prints in the API server terminal.\n")
    stdout.write("\n")
    ...
    try:
        ...
        stderr.write(
            "Try a larger timeout, for example: --timeout 120\n"
        )
        stderr.write(
            "Check the API server terminal for expanded_sparse_search timing logs.\n"
        )
        return 1
